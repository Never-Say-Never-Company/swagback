...
def save_data(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"error": f"Erro interno ao salvar dados: {e}"}, status=500
            )
    ...
    if request.method == "POST":
        try:
            ...
            if not begin or not end:
                return JsonResponse(
                    {"error": "Necessário ter 'begin' e 'end' nas chaves."}, status=400
                )
    ...
    if request.method == "POST":
        try:
            ...
            if not isinstance(data, list):
                return JsonResponse(
                    {"error": "O corpo deve ser uma lista de autores."}, status=400
                )
            ...
            for item in data:
                ...
                if not author:
                    return JsonResponse(
                        {"error": "Necessário ter 'account_id' na chave."}, status=400
                    )
...
def get_project_per_period_and_author(request):
    ...
    try:
        ...
        if not isinstance(authors, list) or not authors:
            return JsonResponse(
                {"error": "O corpo deve conter uma lista não vazia de autores."},
                status=400,
            )
...
def count_issues_grouped_by_project(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Falha ao contar issues por projeto: {e}"}, status=500
        )
...
def paginate_date(request):
    ...
    try:
        ...
        if init is None or end is None:
            return JsonResponse(
                {"Error": "Necessário ter 'init' e 'end' no corpo."}, status=400
            )
        ...
        return JsonResponse({"error": f"Falha ao paginar dados: {e}"}, status=500)
...
def save_developer_rates(request):
    try:
        ...
        if not isinstance(data, list):
            return JsonResponse(
                {
                    "error": "O corpo da requisição deve ser uma lista de desenvolvedores."
                },
                status=400,
            )
