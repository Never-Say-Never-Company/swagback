...
def extract_account_ids(authors_list):
    ...
    return [
        ...
        if isinstance(author, dict) and author.get("account_id")
    ]
...
def get_api_data_issues():
    ...
    response = requests.post(
        API_URL_ISSUES, headers=headers, auth=(API_USER_NAME, API_TOKEN), json=params
    )
    response.raise_for_status()
    return response.json()
        ...
        author_logs.append(
            {
                ...
                "jira_created_at": datetime.strptime(
                    w.get("created").split(".")[0], "%Y-%m-%dT%H:%M:%S"
                ),
...
def get_api_data_users(user_name, token):
    params = {"maxResults": 1000, "orderBy": "displayName"}

    response = requests.get(
        f"{JIRA_URL_USERS}", auth=HTTPBasicAuth(user_name, token), params=params
    )
    response.raise_for_status()
    return response.json()
...
def get_project_by_period(begin, end):
    ...
    end_formated = (
        datetime.strptime(end, "%Y-%m-%d") + timedelta(days=1) - timedelta(seconds=1)
    )
...
def get_project_by_author(accountId):
    results = project_collections.find(
        {
            "issues": {
                "$elemMatch": {"author_logs": {"$elemMatch": {"account_id": accountId}}}
            }
        }
    )
    return list(results)
...
def filther_data(data, author):
    for issue in data["issues"]:
        issue["author_logs"] = [
            log for log in issue["author_logs"] if log["account_id"] == author
        ]
    return data
...
def aggregate_project_periods_and_authors(begin, end, authors):
    ...
    for project in projects_original:
        ...
        if total_team_minutes > 0:
            total_team_aggregation[project_name] = (
                total_team_aggregation.get(project_name, 0) + total_team_minutes
            )

        if selected_people_minutes > 0:
            selected_people_aggregation[project_name] = (
                selected_people_aggregation.get(project_name, 0)
                + selected_people_minutes
            )

    formatted_team_list = []
    for name, minutes in total_team_aggregation.items():
        formatted_team_list.append(
            {"nome_projeto": name, "minutos_projeto": round(minutes)}
        )
...
def list_users_service(filters):
    users = list(
        users_collection.find(
            filters, {"_id": 0, "accountId": 1, "displayName": 1}
        ).sort("displayName", 1)
    )
    return users
...
def list_user_by_Id_service(accountId):
    user = users_collection.find_one(
        {"accountId": accountId}, {"_id": 0, "accountId": 1, "displayName": 1}
    )
    return user
...
def count_issues_grouped_by_project_service():
    pipeline = [
        {"$unwind": "$issues"},
        {"$group": {"_id": "$key", "issue_count": {"$sum": 1}}},
        {"$sort": {"issue_count": -1}},
    ]
    results = list(project_collections.aggregate(pipeline))

    return [
        {"projeto": r["_id"], "quantidade_issues": r["issue_count"]} for r in results
    ]
...
def save_developer_rates_service(data: list) -> dict:
    ...
    for item in data:
        ...
        if not dev_id or rate_value is None:
            raise ValueError(
                "Cada objeto deve ter 'id_desenvolvedor' e 'valor_por_hora'."
            )
    ...
    print(
        f"Documentos deletados na coleção 'developer_rates': {delete_result.deleted_count}"
    )
